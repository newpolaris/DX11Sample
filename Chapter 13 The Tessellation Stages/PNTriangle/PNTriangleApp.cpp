...
class PNTriangleApp : public D3DApp 
{
public:
	PNTriangleApp(HINSTANCE hInstance);
	~PNTriangleApp();
...
private:
	...
	using HShader = std::pair<ComPtr<ID3D11HullShader>, ComPtr<ID3DBlob>>;
	using DShader = std::pair<ComPtr<ID3D11DomainShader>, ComPtr<ID3DBlob>>;
	...
	std::unordered_map<std::string, HShader> mHShaders;
	std::unordered_map<std::string, DShader> mDShaders;
				   ...
				   PSTR cmdLine, int showCmd)
{
	...
	PNTriangleApp theApp(hInstance);
...
PNTriangleApp::~PNTriangleApp()
{
}

bool PNTriangleApp::Init()
{
	if (!D3DApp::Init())
		return false;
...
void PNTriangleApp::BuildFX()
{
	CompileShader(VertexShader, "solid", nullptr, L"FX/PNTriangle.fx", "VS", "vs_5_0");
	CompileShader(PixelShader, "solid", nullptr, L"FX/PNTriangle.fx", "PS", "ps_5_0");
	CompileShader(DomainShader, "TN", nullptr, L"FX/PNTriangle.fx", "DS", "ds_5_0");
	CompileShader(HullShader, "TN", nullptr, L"FX/PNTriangle.fx", "HS", "hs_5_0");
}

void PNTriangleApp::CompileShader(
	...
	case DomainShader:
		{
			...
			md3dDevice->CreateDomainShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, shader.GetAddressOf());
			mDShaders.insert({ name, { shader, blob } });
			break;
		}
	case HullShader:
		{
			ComPtr<ID3D11HullShader> shader;
			md3dDevice->CreateHullShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, shader.GetAddressOf());
			mHShaders.insert({ name, { shader, blob } });
			break;
		}
	}
}

void PNTriangleApp::BuildVertexLayout()
{
...
void PNTriangleApp::BuildMaterials()
{
...
void PNTriangleApp::OnKeyBoardInput(float dt)
{
}
...
void PNTriangleApp::DrawRenderItems(RenderLayer layer)
{
	...
	md3dImmediateContext->HSSetShader(pPSO->pHS, nullptr, 0);
	md3dImmediateContext->DSSetShader(pPSO->pDS, nullptr, 0);
	...
	for (size_t i = 0; i < ritems.size(); ++i) {
		...
		md3dImmediateContext->HSSetConstantBuffers(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT, ConstantBuffers.data());
		md3dImmediateContext->DSSetConstantBuffers(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT, ConstantBuffers.data());
...
void PNTriangleApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void PNTriangleApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void PNTriangleApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void PNTriangleApp::UploadObjects()
{
...
void PNTriangleApp::UpdateMainPassCB(float dt)
{
	...
	MainPassCB.TessFactor = mTessFactor;
...
std::array<const CD3D11_SAMPLER_DESC, 6> PNTriangleApp::GetStaticSamplers()
{
...
void PNTriangleApp::BuildSamplerState() 
{
...
void PNTriangleApp::CreateBlendState(const std::string& tag, const D3D11_BLEND_DESC & desc)
{
...
void PNTriangleApp::CreateDepthStencilState(const std::string& tag, const D3D11_DEPTH_STENCIL_DESC& desc)
{
...
void PNTriangleApp::CreateResterizerState(const std::string& tag, const D3D11_RASTERIZER_DESC& desc)
{
...
void PNTriangleApp::CreatePSO(RenderLayer layer, const PipelineStateDesc& desc)
{
	...
	if (!desc.DS.empty()) {
		assert(mDShaders.count(desc.DS));
		pPSO->pDS = mDShaders[desc.DS].first.Get();
	}
	if (!desc.HS.empty()) {
		assert(mHShaders.count(desc.HS));
		pPSO->pHS = mHShaders[desc.HS].first.Get();
	}
	if (!desc.BS.empty()) {
