...
float PS(VertexOut pin) : SV_Target
{
	...
	float4 centerNormalDepth = gNormalDepthMap.SampleLevel(gSamLinearClamp, pin.Tex, 0.0f);
	...
	for(float i = -gBlurRadius; i <=gBlurRadius; ++i)
	{
		...
		float4 neighborNormalDepth = gNormalDepthMap.SampleLevel(gSamLinearClamp, tex, 0.0f);
