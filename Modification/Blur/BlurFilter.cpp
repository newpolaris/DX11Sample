							 ...
							 int blurCount)
{
	...
	for(int i = 0; i < blurCount; ++i)
	{
		...
		dc->CSSetShaderResources(0, 1, SRVSlot.data());
		ID3D11UnorderedAccessView* UAVSlot[1] = { mBlurredOutputTexUAV };
		dc->CSSetUnorderedAccessViews(0, 1, UAVSlot, nullptr);

		// How many groups do we need to dispatch to cover a row of pixels, where each
		// group covers 256 pixels (the 256 is defined in the ComputeShader).
		UINT numGroupsX = (UINT)ceilf(mWidth / 256.0f);
		dc->CSSetShader(mShader[0], nullptr, 0);
		...
		dc->CSSetShader(mShader[1], nullptr, 0);
...
void BlurFilter::SetShader(std::vector<ID3D11ComputeShader*> shader)
{
	mShader = shader;	
}
