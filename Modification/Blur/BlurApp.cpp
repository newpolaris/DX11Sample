...
private:
	...
	ComPtr<ID3D11RenderTargetView> mOffscreenRTV;
...
void BlurApp::BuildFX()
{
	...
	CompileShader(ComputeShader, "blurVS", nullptr,  L"FX/Blur.fx", "VertBlurCS", "cs_5_0" );
	CompileShader(ComputeShader, "blurHS", nullptr,  L"FX/Blur.fx", "HorzBlurCS", "cs_5_0" );

	mBlur.SetShader( { GetComputeShader("blurHS"), GetComputeShader("blurVS") } );
}

void BlurApp::CompileShader(
...
ID3DBlob* BlurApp::GetVertexShaderBlob(std::string name)
{
	return mVShaders[name].second.Get();
}

ID3D11VertexShader* BlurApp::GetVertexShader(std::string name)
{
	return mVShaders[name].first.Get();
}

ID3D11ComputeShader* BlurApp::GetComputeShader(std::string name)
{
	return mCShaders[name].first.Get();
}

void BlurApp::BuildVertexLayout()
{
...
void BlurApp::OnKeyBoardInput(float dt)
{
}

void BlurApp::UpdateCamera(float dt)
{
...
void BlurApp::DrawRenderItems(RenderLayer layer)
{
...
void BlurApp::DrawScene()
{
	...
	std::vector<ID3D11RenderTargetView*> rt = { mOffscreenRTV.Get() };
	...
	md3dImmediateContext->PSSetShaderResources(0, 2, null);

	HR(mSwapChain->Present(0, 0));
}

void BlurApp::DrawScreen()
{
	DrawRenderItems(RenderLayer::Screen);
}
...
void BlurApp::BuildShapeGeometryBuffers()
{
...
void BlurApp::BuildSkullGeometryBuffers()
{
...
void BlurApp::BuildRenderItems()
{
	...
	AddRenderItems(Screen, "screen", "screen", "screenMat");
}

void BlurApp::AddRenderItems(
	RenderLayer layer,
	std::string geoname,
	std::string subname,
	std::string matname,
...
void BlurApp::UpdateMainPassCB(float dt)
{
...
void BlurApp::BuildOffScreenBuffer()
{
	...
	md3dDevice->CreateTexture2D(&desc, nullptr, texture.GetAddressOf());

	HR(md3dDevice->CreateShaderResourceView(texture.Get(), nullptr, mOffscreenSRV.ReleaseAndGetAddressOf()));
	HR(md3dDevice->CreateUnorderedAccessView(texture.Get(), nullptr, mOffscreenUAV.ReleaseAndGetAddressOf()));
	HR(md3dDevice->CreateRenderTargetView(texture.Get(), nullptr, mOffscreenRTV.ReleaseAndGetAddressOf()));

	auto screenTex = std::make_unique<Texture>();
	screenTex->Resource = mBlur.GetBlurredOutput();
	mTextures["screen"] = std::move(screenTex);
}

std::array<const CD3D11_SAMPLER_DESC, 6> BlurApp::GetStaticSamplers()
{
...
void BlurApp::BuildSamplerState() 
{
...
void BlurApp::CreateBlendState(const std::string& tag, const D3D11_BLEND_DESC & desc)
{
...
void BlurApp::CreateDepthStencilState(const std::string& tag, const D3D11_DEPTH_STENCIL_DESC& desc)
{
...
void BlurApp::CreateResterizerState(const std::string& tag, const D3D11_RASTERIZER_DESC& desc)
{
...
void BlurApp::CreatePSO(RenderLayer layer, const PipelineStateDesc& desc)
{
...
void BlurApp::BuildPSO()
{
	...
	CreatePSO(RenderLayer::Screen, { "texture", "screen", "screen" });
}
